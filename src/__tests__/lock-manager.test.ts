...
describe("LockManager", () => {
  it("acquires lock when no conflict", () => {
    ...
    const lock = lm.acquire("host", "host-1", "agent-1", "task-1")
    assert.ok(lock)
  ...
  it("rejects lock when host already locked by another agent", () => {
    ...
    const lock2 = lm.acquire("host", "host-1", "agent-2", "task-2")
    assert.equal(lock2, null)
  })

  it("allows same agent to renew lock", () => {
    const lm = new LockManager()
    const lock1 = lm.acquire("host", "host-1", "agent-1", "task-1")
    const lock2 = lm.acquire("host", "host-1", "agent-1", "task-1")
    assert.ok(lock2)

...
Scheduler state is persisted under `~/.ssh-tool/scheduler/`, so multiple MCP sessions can see what is running, what is queued, and what recently completed. Use `ssh_queue_status`, `ssh_wait_task`, and `ssh_exec_status` instead of rerunning queued or timed-out commands.
...
Recipes are persisted under the scheduler state directory (`~/.ssh-tool/scheduler/state/commands.json` by default). Register/update/delete operations take a small cross-process file lock and re-read the latest file before writing, so separate MCP sessions do not normally lose each other's command changes.
...
调度状态持久化在 `~/.ssh-tool/scheduler/` 下。多个 MCP 会话可以看到当前 running、queued、recent 任务。queued 或 wait timeout 后，应使用 `ssh_queue_status`、`ssh_wait_task`、`ssh_exec_status` 查询同一个 `taskId`，不要重复提交命令。
...
默认存储位置是 scheduler 状态目录（通常为 `~/.ssh-tool/scheduler/state/commands.json`）。`ssh_command_register` / `ssh_command_update` / `ssh_command_delete` 写入时会拿跨进程文件锁，并在锁内重新读取最新文件再合并写回，避免多个 MCP 会话同时保存时互相覆盖。

...
`ssh_cd` sets a per-agent, per-host virtual cwd. It does not affect other AI sessions. Future `ssh_exec` and `ssh_schedule` calls use it unless `cwd` is provided explicitly.
...
`ssh_cd` 设置当前 AI 会话在某个 host 上的虚拟 cwd，不影响其他 AI。后续 `ssh_exec` / `ssh_schedule` 没显式传 `cwd` 时会使用它。

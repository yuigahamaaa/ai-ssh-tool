...
| **MCP Server** | Standard Model Context Protocol — 31 tools for AI agents |
...
`ssh_cd` sets a per-agent, per-host virtual cwd. It is not a persistent remote shell `cd`; it only stores cwd metadata that later `ssh_exec` and `ssh_schedule` calls use when `cwd` is omitted. It does not affect other AI sessions or shared SSH sessions.
...
| Virtual cwd | `ssh_cd` | Set this AI session's virtual cwd; not a persistent remote shell `cd` |
...
`ssh_cd` 设置当前 AI 会话在某个 host 上的虚拟 cwd。它不是远端 shell 的持久 `cd`，只是保存 cwd 元数据；后续 `ssh_exec` / `ssh_schedule` 没显式传 `cwd` 时会使用它。它不影响其他 AI 或共享 SSH 会话。
...
| 虚拟 cwd | `ssh_cd` | 设置当前 AI 会话的虚拟 cwd；不是远端 shell 的持久 `cd` |
